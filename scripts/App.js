...
export default function FuchsGame() {
    ...
    const isLegalPlay = useCallback((card, hand, trick, rotIsTrump, calledCard) => {
        ...
        // Rule about the "Called Ace" (Gsuchte)
        if (calledCard && card.id === calledCard.id) {
            if (getEffectiveSuit(card, rotIsTrump) !== leadSuit && trick.length > 0) return false;
        }
    ...
    const processCall = (callType, cardObject = null) => {
        ...
        setGameState(prev => ({
            ...
            status: callType === 'Rot_is_trump' ? "Rot is Trumpf!" : `Called: ${cardObject.suit} ${cardObject.rank}`
        }));
    };
    ...
    const playCard = (playerIndex, cardIndex) => {
        ...
        if (!isLegalPlay(card, player.hand, gameState.trick, gameState.RotIsTrump, gameState.calledCard)) {
    ...
    useEffect(() => {
        ...
        if (gameState.phase === 'gameplay' && gameState.currentPlayer !== 0 && gameState.trick.length < 4) {
            const timer = setTimeout(() => {
                ...
                const legalIdx = hand.findIndex(c => isLegalPlay(c, hand, gameState.trick, gameState.RotIsTrump, gameState.calledCard));
                    ...
                    <AnimatePresence>
                        {gameState.trick.map((t, i) => (
                            <motion.div
                                ...
                                className={`played-card pos-${t.player}`}
                            >
        ...
        <div className="overlay">
            <div className="modal-content">
                ...
                <div className="hand-preview">
                    {hand.map(c => <img key={c.id} src={c.image} className="mini-card" alt="preview" />)}
                </div>
            </div>
        </div>
    );
